...
| 결제 담당자 | 이름, 이메일, 휴대폰 번호 | 저장. 카드 본인인증, 청구·결제 관련 안내에 이용 |
| 결제 프로필 **(선택)** | 회사명, 주소 | 저장. 세금계산서·영수증에 표기되는 청구처 정보 |
| 거래 기록 | 결제, 환불, 취소·정정 등 거래 내역 | 관련 법령에 따른 보존 기간 동안 저장(제3조 참조) |
...
4. 결제수단 등록 및 결제 과정에서 사용자가 직접 입력하여 수집
5. 웹사이트의 문의 폼을 통해 문의자가 직접 입력
...
4. **제3자 기술 제공자의 학습 이용:** 각 제공업체의 학습 이용 여부 및 보유 기간은 해당 업체의 데이터 처리 정책에 따르며, 본 처리방침 제4조 및 서비스 내 [데이터 처리 위탁 현황 페이지](https://docs.platform.letsur.ai/policies/subprocessors)에서 확인할 수 있습니다.
...
본 처리방침은 회사가 제공하는 모든 서비스에 공통 적용되며, 개별 서비스(AI 게이트웨이(AI Gateway), 결제(Billing), 분석(Analytics) 등)에서 추가로 수집·처리하는 개인정보가 있는 경우 해당 서비스의 특약 또는 [이용약관](https://platform.letsur.ai/terms)에서 별도로 정합니다. 본문과 특약이 충돌할 경우 특약이 우선 적용됩니다.
...
| 고객 응대 도구 제공업체 | 문의 접수·처리 및 내부 알림 | 위탁 계약 종료 시까지 |
...
4. **결제 정보 보안:** 카드 등록 시 입력된 카드 정보는 암호화 전송(TLS)으로 전자결제 대행사에 전달되어 빌링키(결제용 토큰) 발급에 사용되며, 회사는 카드번호와 카드 비밀번호를 저장하거나 기록하지 않으며, 카드 관련 정보로는 카드사·카드번호 끝 4자리·빌링키만 보관합니다.
